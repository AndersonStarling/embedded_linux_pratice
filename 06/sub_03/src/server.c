#include <stdio.h>
#include <stdbool.h>
#include <sys/socket.h>
#include <sys/un.h>
#include <arpa/inet.h>
#include <unistd.h>
#include <errno.h>

typedef bool (*function_handler)(void *);

typedef struct
{
    function_handler function;
    void * args;
} function_sequence;

typedef struct
{
    int socket_fd;
    int client_fd;
    ssize_t rx_byte_len;
    ssize_t tx_byte_len;
    char rx_buffer[1024];
} socket_runtime_struct_t;

typedef struct
{
    int domain;
    int type;
    int protocol;
    int sock_fd;
} socket_struct_t;

typedef struct
{
    int back_log;
} socket_listen_struct_t;

typedef struct 
{
    struct sockaddr_un socket_address;
    struct sockaddr_un client_address;
    char * socket_path;
    char * client_path;
} bind_socket_struct_t;


static bool create_socket(void * args);
static bool bind_socket(void * args);
static bool listen_from_socket(void * args);
static bool accept_connect_socket(void * args);
static bool receive_from_socket(void * args);
static bool send_to_socket(void * args);
static bool close_socket(void * args);

socket_runtime_struct_t common_data;

socket_struct_t create_socket_data = 
{
    .domain   = AF_UNIX,
    .type     = SOCK_STREAM,
    .protocol = 0,
};

bind_socket_struct_t bind_socket_data = 
{
    .socket_address.sun_family = AF_UNIX,
    .socket_path = "/tmp/server.sock",
    .client_address.sun_family = AF_UNIX,
    .client_path = "/tmp/client.sock",
};

socket_listen_struct_t listen_socket_data = 
{
    .back_log = 10,
};

function_sequence sequence[] = 
{
    [0] = 
    {
        .function = create_socket,
        .args     = (void *)&create_socket_data
    },
    [1] =
    {
        .function = bind_socket,
        .args     = (void *)&bind_socket_data
    },
    [2] =
    {
        .function = listen_from_socket,
        .args     = (void *)&listen_socket_data
    },
    [3] =
    {
        .function = accept_connect_socket,
        .args     = (void *)&bind_socket_data
    },
    [4] = 
    {
        .function = receive_from_socket,
        .args     = NULL
    },
    [5] =
    {
        .function = send_to_socket,
        .args     = NULL
    },
    [6] =
    {
        .function = close_socket,
        .args     = NULL
    }
};

static bool create_socket(void * args)
{
    bool ret_val = false;

    printf("%s\n", __func__);

    socket_struct_t * socket_data = (socket_struct_t *)args;

    common_data.socket_fd = socket(socket_data->domain, socket_data->type, socket_data->protocol);

    ret_val = (common_data.socket_fd > 0);

    if(common_data.socket_fd < 0)
    {
        printf("%s failed\n", __func__);
    }

    return ret_val;
}

static bool bind_socket(void * args)
{
    bind_socket_struct_t * bind_socket_data = (bind_socket_struct_t *)args;
    int ret;
    bool ret_val = false;

    printf("%s\n", __func__);

    unlink(bind_socket_data->socket_path);

    /* configure server path */
    strncpy(bind_socket_data->socket_address.sun_path, bind_socket_data->socket_path, sizeof(bind_socket_data->socket_address.sun_path) - 1);

    ret = bind(common_data.socket_fd,                                      \
               (const struct sockaddr *)&bind_socket_data->socket_address, \
               sizeof(bind_socket_data->socket_address));

    ret_val = (ret == 0);

    if(ret != 0)
    {
        printf("ret = %d\n", ret);
        printf("%s failed\n", __func__);
    }

    return ret_val;
}

static bool listen_from_socket(void * args)
{
    socket_listen_struct_t * listen_socket_data = (socket_listen_struct_t *)args;
    bool ret_val = false;
    int ret;

    printf("%s\n", __func__);

    ret = listen(common_data.socket_fd, listen_socket_data->back_log);
    if(ret != 0)
    {
        printf("%s failed\n", __func__);
    }

    ret_val = (ret == 0);

    return ret_val;
}

static bool accept_connect_socket(void * args)
{
    bind_socket_struct_t * bind_socket_data = (bind_socket_struct_t *)args;
    bool ret_val = false;
    int ret;

    printf("%s\n", __func__);

    socklen_t addrlen = sizeof(bind_socket_data->client_address);
    common_data.client_fd = accept(common_data.socket_fd,                                \
                                   (struct sockaddr *)&bind_socket_data->client_address, \
                                   &addrlen);
    if(ret < 0)
    {
        printf("%s failed\n", __func__);
    }
                
    ret_val = (ret > 0);

    return ret_val;
}

static bool receive_from_socket(void * args)
{
    (void)args;
    bool ret_val = false;

    printf("%s\n", __func__);

    common_data.rx_byte_len = read(common_data.client_fd, &common_data.rx_buffer[0], sizeof(common_data.rx_buffer));

    printf("%s: %s", __func__, &common_data.rx_buffer[0]);

    ret_val = (common_data.rx_byte_len > 0);

    return ret_val;
}

static bool send_to_socket(void * args)
{
    (void)args;
    bool ret_val = false;

    printf("%s\n", __func__);

    common_data.tx_byte_len = write(common_data.client_fd, &common_data.rx_buffer[0], sizeof(common_data.rx_buffer));
                                    
    ret_val = (common_data.tx_byte_len > 0);

    return ret_val;
}

static bool close_socket(void * args)
{
    printf("%s\n", __func__);
    (void)args;
    close(common_data.socket_fd);

    return true;
}


int main(void)
{
    int index = 0;

    for(index = 0; index < sizeof(sequence)/sizeof(function_sequence); index ++)
    {
        if(sequence[index].function(sequence[index].args) == false)
        {
            /* call last function in case any function failed */
            sequence[sizeof(sequence)/sizeof(function_sequence) - 1].function(sequence[sizeof(sequence)/sizeof(function_sequence) - 1].args);
            return -1;
        }
    }

    return 0;
}

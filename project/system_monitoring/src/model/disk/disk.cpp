...
bool disk::valid_disk(char * buffer, uint16_t buffer_size)
{
    ...
    ret = !((name.rfind("loop", 0) == true)   || \
          (name.rfind("ram", 0)  == true)     || \
          (name.rfind("fd", 0)   == true)     || \
          (name.rfind("sr", 0)   == true));

    return ret;
}


uint64_t disk::get_disk_read_speed(void)
{
    int ret;
    FILE *fp;
    ...
    for(;  valid_disk(buffer, BUFFER_SIZE) == false; )
    {
        fgets(&buffer[0], BUFFER_SIZE, fp);
    }
    ...
    return (float)((delta_read) * 512) / (1024 * 1024 * 2);

}

uint64_t disk::get_disk_write_speed(void)
{
    int ret;
    FILE *fp;
    char buffer[BUFFER_SIZE] = {0};
    ...
    return (float)((delta_write) * 512) / (1024 * 1024 * 2);
